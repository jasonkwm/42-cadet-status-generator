...
3. First `git clone` into a folder like so: (replace `folder-name` if you want)
   
   ```
   git clone https://github.com/Bunyod16/42-cadet-status-generator.git folder-name
   ```
...
4. Once you've cloned the repository go into the folder by typing
   ...
   - After you have `cd` into the folder you can open the folder and see whats inside <br />
...
5. To Make Everything Work, go back to the Terminal:
   ...
   - Once Virtual Environment is generated, start up the venv
...
6. Once the script is running you have to get your `UID` & `SECRET` keys from intra :
